...
**M0–M10 离线内核已全部落地（离线优先，197 测试全绿）**：契约 → PIT 数据 + 回测 → 防过拟合评测 → LLM 信号层（离线 stub）→ 决策 + 预交易风控门 + 模拟盘闭环 → 实验框架 + regime/drift 监控 →（M7）AI gateway/成本预算/优先级节流/PIT 新闻源/真实后端适配 →（M8）高保真 broker（费用/滑点/延迟/部分成交）+ 回测-实盘 parity →（M9）度量/告警/tracing/容器化 →（M10）上线闸门/放量回滚/防篡改审计。代码在 `src/atrading/`。
