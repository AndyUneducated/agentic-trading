...
> **状态**：M1–M10 **离线内核均已实现并全绿**（189 测试，离线优先）；示例代码与实际实现基本一致。M7–M10 的"实现状态"小节标注了**已落地的离线部分**与**待真实基建的部分**（真实 LLM/数据/券商、Nautilus、Grafana/OTLP/密钥托管、合规与实盘签署），后者收敛到"人类开关"后（见 [ADR-0009](../decisions/0009-offline-first-productionization.md)、[PRODUCTION-READINESS.md](../PRODUCTION-READINESS.md)）。
