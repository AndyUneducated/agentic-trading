...
# 存储 schema 版本：变更磁盘布局/列时递增，read 端据此拒绝不兼容数据（预留迁移钩子）。
SCHEMA_VERSION = 1
...
class PITStore:
    ...
    def _schema_path(self) -> Path:
        return self.root / "bars" / "_schema.json"

    def schema_version(self) -> int | None:
        """已落盘数据的 schema 版本；无数据时返回 None。"""
    ...
    def _check_schema(self) -> None:
        ...
        if version is not None and version != SCHEMA_VERSION:
            msg = f"PIT store schema v{version} 与代码 v{SCHEMA_VERSION} 不一致，需迁移"
            raise DataError(msg)
